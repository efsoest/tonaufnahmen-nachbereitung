...
describe('End-to-End Processing Test', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(path.join(veranstaltungenDir, '01_Predigt_Test.mp3'), dummyMp3);
    fs.writeFileSync(path.join(veranstaltungenDir, '02_Lobpreis_Test.mp3'), dummyMp3);
  ...
  it('should tag, rename and copy fake MP3 to local target drive folder with cover', async () => {
    ...
    const tagSuccess = writeId3Tags(mp3, metadata, event.date, event.displayName);
